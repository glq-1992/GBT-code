...
for line in dev.readlines():
    line = line.replace('\n', '')
    index,name,length,gloss,char,word,question= line.split('|')[0:7]
    dev_text_list.append(word)
    dev_question_list.append(question)
    # for i in text.split(' '):
...
labels= ['BLEU1','BLEU2','BLEU3','BLEU4']
...
plt.show()
plt.savefig('dev_test_overfit_csldaily.png')
