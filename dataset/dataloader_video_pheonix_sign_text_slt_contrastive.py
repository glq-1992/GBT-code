...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, text_dict,tokenizer,drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
        ...
        data_txt = f"/home/gaoliqing/shipeng/code/slrBert/DatasetFile/dialogue_pheonix/{mode}_contrastive.txt" #yuanlai
        # data_txt = f"/home/gaoliqing/code_glq/CVPR_2022/context_csl-daily_contrastive/DatasetFile/dialogue_pheonix_zekang/{mode}.txt" #zekang
        ...
        with open(data_txt) as f:
            for i,line in enumerate(f.readlines()):
                ...
                line = line.replace('\n', '')
                name, gloss, question, question_neg1, textLabel, question_neg2 = line.split('%')
                ...
                # self.inputs_list[i] = dict(folder=folder,label=label,textLabel = textLabel, text=text,next_sentence_label=1,original_info=folder)
                self.inputs_list[i] = dict(folder=name,label=gloss,textLabel = textLabel, text=question,text_negative1=question_neg1,text_negative2=question_neg2,next_sentence_label=1,original_info=name)
    ...
    def __getitem__(self, idx):
        # input_data, label, text_label_list, text,next_sentence_label, fi = self.read_video(idx)
        ...
        # return input_data, torch.LongTensor(label), text_label_list, text,next_sentence_label,self.inputs_list[idx]['original_info'],'test'

        return input_data, torch.LongTensor(label), text_label_list, text, text_neg1, text_neg2, next_sentence_label, \
               self.inputs_list[idx]['original_info'], 'test'


    def read_video(self, index):
        fi = self.inputs_list[index]
        # img_folder = fi['folder']
        img_folder =  self.dataset_root + fi['folder'] + '/*.png'
        ...
        return [cv2.cvtColor(cv2.imread(i), cv2.COLOR_BGR2RGB) for i in
                img_list], label_list, text_label_list, input_ids, input_ids_neg1, input_ids_neg2, fi[
                   "next_sentence_label"], fi
        ...
        # input_ids = self.tokenizer.convert_tokens_to_ids(text_list)
        #
        # return [cv2.cvtColor(cv2.imread(i), cv2.COLOR_BGR2RGB) for i in img_list], label_list, text_label_list, input_ids,fi["next_sentence_label"], fi
    ...
    #
    #     batch = [item for item in sorted(batch, key=lambda x: len(x[0]), reverse=True)]
    #     video, label, text_label ,text , next_sentence_label,info,_ = list(zip(*batch))
    #     if len(video[0].shape) > 3:
    #         max_len = len(video[0])
    #         video_length = torch.LongTensor([np.ceil(len(vid) / 4.0) * 4 + 12 for vid in video])
    #         left_pad = 6
    ...
    #         padded_next_sentence_label.append(i)
    #     padded_next_sentence_label = torch.LongTensor(padded_next_sentence_label)
    #     return padded_video, video_length, padded_label, label_length, padded_text_label,text_label_length,padded_text_label_target,padded_text,text_length,padded_next_sentence_label, info
