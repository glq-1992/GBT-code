...
def ctc_loss(log_probs : torch.Tensor, targets : torch.Tensor, input_lengths : torch.Tensor, target_lengths : torch.Tensor, blank : int = 0, reduction : str = 'none', finfo_min_fp32: float = torch.finfo(torch.float32).min, finfo_min_fp16: float = torch.finfo(torch.float16).min, alignment : bool = False):
	with torch.autograd.set_detect_anomaly(True):
		...
		for t in range(1, input_time_size):
			log_alpha[t, :, 2:] = log_probs_[t] + logadd(log_alpha[t - 1, :, 2:].clone(), log_alpha[t - 1, :, 1:-1].clone(), torch.where(diff_labels, log_alpha[t - 1, :, :-2].clone(), zero))
...
def ctc_alignment_targets(log_probs, targets, input_lengths, target_lengths, blank = 0, ctc_loss = F.ctc_loss, retain_graph = True):
	...
	grad_log_probs, = torch.autograd.grad(loss, log_probs, retain_graph = retain_graph,allow_unused=True)
	grad_logits = grad_log_probs - probs * grad_log_probs.sum(dim = -1, keepdim = True)
	input_lengths = input_lengths.to(targets.device)
	temporal_mask = (torch.arange(len(log_probs), device = targets.device, dtype = input_lengths.dtype).unsqueeze(1) < input_lengths.unsqueeze(0)).unsqueeze(-1)
