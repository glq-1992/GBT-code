...
faulthandler.enable()
import utils
from seq_scripts_GSL_bert_text_nsp_slt_german_bias_slr_gai import seq_train, seq_eval, seq_feature_generation,seq_eval_slr # seq_scripts_GSL_bert_text_nsp_slt_vg, seq_scripts_GSL_bert_text_nsp_slt_german_bias_slr_gai
...
class Processor():
    def __init__(self, arg):
        ...
        with open(ch_vocab_text_filename, encoding='utf-8') as f:
            vocab = f.readlines()
            self.text_dict = {vocab[i].replace('\n', ''):i+3 for i in range(len(vocab))}
            self.text_dict[PAD_TOKEN] = 0
            self.text_dict[BOS_TOKEN] = 1
            self.text_dict[EOS_TOKEN] = 2
        ch_vocab_filename_q = "DatasetFile/dialogue_sp_10/t_vocab_zh.txt"
        with open(ch_vocab_filename_q, encoding='utf-8') as f:
            vocab = f.readlines()
            self.text_dict_q = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))}
            self.text_dict_q[PAD_TOKEN] = 0
    ...
    def load_data(self):
        ...
        for idx, (mode, train_flag) in enumerate(dataset_list):
            ...
            self.dataset[mode] = self.feeder(gloss_dict=self.gloss_dict,text_dict = self.text_dict,text_dict_q = self.text_dict_q,tokenizer=tokenizer, **arg)
...
if __name__ == '__main__':
    ...
    sparser = utils.parameters_sp10_bert_sign_text_graph_slt.get_parser()
