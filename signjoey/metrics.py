...
def word_accuracy(references, hypotheses, level="word"):
    """
    Compute the accuracy of hypothesis tokens: correct tokens / all tokens
    Tokens are correct if they appear in the same position in the reference.
    ...
    for hyp, ref in zip(hypotheses, references):
        all_tokens += len(hyp)
        w1 = [w for w in hyp.split(split_char)]
        w2 = [w for w in ref.split(split_char)]
...
# def sentence_accuracy(references, hypotheses):
#     """
#     Compute the accuracy of hypothesis tokens: correct tokens / all tokens
#     Tokens are correct if they appear in the same position in the reference.
#
...
#     acc = 0
#     assert len(hypotheses) == len(references)
#     w1 = [w for w in hypotheses.split('')]
#     w2 = [w for w in references.split('')]
