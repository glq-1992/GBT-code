...
from signjoey.decoders import Decoder, TransformerDecoder, TransformerDecoder2Encoder
...
def greedy(
    ...
    elif isinstance(decoder, TransformerDecoder2Encoder):
        # Transformer greedy decoding
        greedy_fun = transformer_greedy_2Encoder
    else:
...
def transformer_greedy(
    ...
    for _ in range(max_output_length):
        # bert embedding的输入方式
        trg_embed = embed(None,None,ys)  # embed the previous tokens
...
def transformer_greedy_2Encoder(
    src_mask1: Tensor,
    src_mask2: Tensor,
    embed: Embeddings,
    bos_index: int,
    ...
    batch_size = src_mask1.size(0)

    # start with BOS-symbol for each sentence in the batch
    ys = encoder_output1.new_full([batch_size, 1], bos_index, dtype=torch.long)

    # a subsequent mask is intersected with this in decoder forward pass
    trg_mask = src_mask1.new_ones([1, 1, 1])
    finished = src_mask1.new_zeros((batch_size)).byte()
...
def transformer_greedy_3Encoder(
    src_mask1: Tensor,
    src_mask2: Tensor,
    src_mask3: Tensor,
    embed: Embeddings,
    ...
    for _ in range(max_output_length):
        ...
        with torch.no_grad():
            logits, out, _, _ = decoder(
                ...
                encoder_output1=encoder_output1,
                encoder_output2=encoder_output2,
                encoder_output3 = encoder_output3,
                encoder_hidden=None,
                src_mask1=src_mask1,
                src_mask2=src_mask2,
                src_mask3 = src_mask3,
