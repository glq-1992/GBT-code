...
class FeatureDisentangle(nn.Module):
    ...
    def forward(self, feature_combine):
        ...
        mask = self.relu(mask)
        mask = self.fc_1(mask.transpose(1,2))
        mask = self.fc_2(mask)
        mask = self.sigmoid(mask)

        # mask_one = torch.ones_like(mask)
        feature_one = torch.einsum('ijk,ijk->ijk',[mask,feature_combine.transpose(1,2)])
        feature_two = torch.einsum('ijk,ijk->ijk',[(1-mask),feature_combine.transpose(1,2)])
